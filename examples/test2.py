...
if __name__ == "__main__":
    ...
    for eps in eps_values:
        try:
            mix_grid = dd_optimal.dbscan_shells(gmm=gmm, eps=eps, min_samples=10, plot=False)
            disc_mix, w2_mix = dd.discretize(gmm, mix_grid)
    ...
    if best_eps is not None:
        ...
        print("\nNo valid eps found! Consider adjusting the eps_values range or lowering min_samples.")

    # using mix_grids with domain(s) generated by DBSCAN
    mix_grid = dd_optimal.dbscan_shells(gmm=gmm, eps=best_eps, min_samples=10, plot=True)
    disc_mix, w2_mix = dd.discretize(gmm, mix_grid)
    print(f'W2 (MultiGridScheme from dbscan_shells): {w2_mix.item()}')
    print(f'nr locs mix grid {len(disc_mix.locs)}')
    fig, ax = plt.subplots(figsize=(8, 8))

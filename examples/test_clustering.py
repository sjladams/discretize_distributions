...
if __name__ == "__main__":
    ...
    # component_distribution = dd_dists.MultivariateNormal(loc=torch.tensor([[0.0, 0.0]]),
    #                                                      covariance_matrix=torch.diag_embed(torch.tensor([[1., 1.]])))
    # mixture_distribution = torch.distributions.Categorical(probs=torch.tensor([.5]))
    # # mixture_distribution = torch.distributions.Categorical(probs=torch.tensor([.3, .8, .6, 0.1]))
    # gmm = dd_dists.MixtureMultivariateNormal(mixture_distribution, component_distribution)

    component_distribution = dd_dists.MultivariateNormal(loc=torch.tensor([[5.0, 5.0], [-5.0, -5.0]]),
                                                         covariance_matrix=torch.diag_embed(torch.tensor([[1., 1.],
                                                                                                          [1., 1.]])))
    ...
    shells, centers, eps = dd_optimal.dbscan_shells(gmm=gmm)
    mix_grid = dd_optimal.create_grid_from_shells_old(gmm, shells, centers, eps)
    ...
    component_distribution = dd_dists.MultivariateNormal(loc=torch.tensor([[5.0, 5.0], [4.0, 4.0]]),
                                                         covariance_matrix=torch.diag_embed(torch.tensor([[1., 1.],
                                                                                                          [1., 1.]])))
    mixture_distribution = torch.distributions.Categorical(probs=torch.tensor([.5, .5]))
    # mixture_distribution = torch.distributions.Categorical(probs=torch.tensor([.3, .8, .6, 0.1]))
    ...
    shells, centers, eps = dd_optimal.kmeans_shells(gmm=gmm)
    mix_grid = dd_optimal.create_grid_from_shells(gmm, shells, centers)
    disc_mix, w2_mix = dd.discretize(gmm, mix_grid)

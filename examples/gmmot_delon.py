...
def densite_theorique(mu,var,alpha,x):
    # compute the 1D GMM density with parameters (mu,var) and weights alpha  at x --> discrete
    K=mu.shape[0]
    y=0
    #y=np.zeros(len(x))
    for j in range(K):
...
def GW2(pi0,pi1,mu0,mu1,S0,S1):
    ...
    wstar     = ot.emd(pi0,pi1,M)         # discrete transport plan for each pairwise Gaussian, combined back into one matrix
    distGW2   = np.sum(wstar*M)           # this is their Wasserstein distance (MW2) - check!

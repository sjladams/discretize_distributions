...
if __name__ == "__main__":
    ...
    #
    # disc_norm, w2, _ = dd.discretize(norm, grid_scheme)
    #
    ...
    #
    # optimal_disc_norm, w2, _ = dd.discretize(norm, optimal_grid_scheme)
    #
    # fig, ax = plt.subplots(figsize=(8, 8))
    # ax = plot_2d_dist(ax, norm)
    ...
    grid_partition = dd_schemes.GridPartition.from_grid_of_points(grid_locs)
    grid_scheme = dd_schemes.GridScheme(grid_locs, grid_partition)
    disc, w2, _ = dd.discretize(gmm, grid_scheme)
    print(f'w2:{w2}')

    fig, ax = plt.subplots(figsize=(8, 8))
    ax = plot_2d_dist(ax, gmm)
    ax = plot_2d_cat_float(ax, disc)
    ax = set_axis(ax)
    ax.set_title(f'One uniform grid scheme for whole space: {w2.item():.2f}')
    plt.show()
    ...
    #
    # for i in range(num_mix_elems):
    #     lower_vertex, upper_vertex = vertices_list[i]
    ...
    #         gmm.component_distribution[i], num_locs=num_locs, domain=domain
    #     )
    #
    ...
    ax = plot_2d_cat_float(ax, disc)
    ax = set_axis(ax)
    ax.set_title(f'Mix schemes using optimal locations: {w2.item():.2f}')
    plt.show()

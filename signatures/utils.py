...
from distributions.utils import cdf, pdf, mean_uni_trun_gauss, var_uni_trun_gauss, _ym2, mean_and_var_uni_trun_gauss
import distributions
from tensor.utils import make_sym
...
def get_disc(norm: Union[distributions.MultivariateNormal, distributions.MixtureMultivariateNormal],
    ...
    # eigvals, eigvectors = tensor.utils.eigh_block_structure(matrix=norm.covariance_matrix,
    #                                                         block_size=path_length)
    # get transformation matrix
    # via cholesky decomposition
    # L = torch.linalg.cholesky(norm.covariance_matrix)
    # compute inverse efficiently
    # T = torch.linalg.solve_triangular(L, torch.eye(L.shape[-1])[None, None].expand(L.shape), upper=False)
    # check = torch.einsum('...ij,...jk,...lk->...il', T, norm.covariance_matrix, T)
    # T2norm = torch.linalg.norm(T, ord=2, dim=(-2,-1))
    # torch.diagonal(torch.inverse(torch.einsum('...ij,...jk->...ik', T, T)), dim1=-1, dim2=-2)
    # eigvals = torch.diagonal(torch.einsum('...ij,...jk->...ik', L, L), dim1=-1, dim2=-2)
...
def get_disc_locs_stand_normal_axes(nr_signature_points_per_axes_per_side: int, nr_dims: int, prob_center: float = 0.):
    ...
    if not via_pattern:
        ...
        if pattern_type == 'step':
            ...
            pattern = tune_quadratic_distibution_locs(nr_signature_points_per_axes_per_side, skew=0.) # if skew = 0, ps are constant
        ...
        volume_hyper_rects_dum = (pattern * (1 - prob_center) + prob_center * nr_signature_points_per_axes_per_side) / nr_signature_points_per_axes_per_side
...
def get_disc_axes(eigvals: torch.Tensor, eigvectors: torch.Tensor, mean: torch.Tensor, nr_signature_points_per_axes: int,
    ...
    if nr_signature_points_per_axes == 1:
        ...
        cs = torch.cat((-cs.flip(-2), cs), dim=-2) # flip? (if decide to do so, also flip ps!) (only provides structure for debug)
        cs = cs + mean.unsqueeze(-2)

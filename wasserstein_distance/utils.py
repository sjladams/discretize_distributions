...
def to_std_rv(loc, scale, value):
    return (value - loc) * scale.reciprocal()

def little_phi(loc, scale, x):
    return (-to_std_rv(loc, scale, x).pow(2) * 0.5).exp() * CONST_INV_SQRT_2PI * scale.reciprocal()


def big_phi(loc, scale, x):
    return 0.5 * (1 + (to_std_rv(loc, scale, x) * CONST_INV_SQRT_2).erf())
...
#     Tx = m1 + (x - m0) @ Sigma
#
#     Sigma0_inv_sqrtm = np.linalg.inv(spl.sqrtm(Sigma0))
#     Sigma0_sqrtm = spl.sqrtm(Sigma0)
#     Sigma = Sigma0_inv_sqrtm @ spl.sqrtm(Sigma0_sqrtm@Sigma1@Sigma0_sqrtm) @ Sigma0_inv_sqrtm
#
#     Tx = m1 + (x - m0) @ Sigma
#
#     return Tx
...
#                 for i in range(n):
#                     # Ti = int(max(min(T[k, l, i], x[-1]), x[0]) * (n-1))
#                     Ti = int(np.round(((max(min(T[k, l, i], x[-1]), x[0, 0]) - x[0, 0]) / (x[-1,0] - x[0, 0])) * (n-1)))
...
#
#     index = 0;
#     A = np.zeros((nConstraints, nParameters));
...
#
#     out = big_uint[2 * r + 1:nrow + 2 * r + 1, 2 * r + 1:ncol + 2 * r + 1] + big_uint[0:nrow, 0:ncol] - big_uint[0:nrow,
#                                                                                                         2 * r + 1:ncol + 2 * r + 1] - big_uint[
#                                                                                                                                       2 * r + 1:nrow + 2 * r + 1,
#                                                                                                                                       0:ncol]
#     out = out / (2 * r + 1) ** 2
#
...
#     # pi0 = np.array([.3, .7])
#     mu0 = np.array([[0.3, 0.5, 0.7]]).T
#     # mu0 = np.array([[.2, .4]]).T
#     # S0  = np.array([[[.0009]],[[.0016]]])
#     S0 = np.array([[[0.002]], [[0.002]], [[.002]]])
#     K0 = pi0.shape[0]
#
#     # second GMM
#     # pi1 = np.array([.6, .4])
...
#     Tmap, _ = GW2_map(pi0, pi1, mu0, mu1, S0, S1, wstar, x)
#     plt.figure(3, figsize=(4, 4))
#     plot1D_mat(gmm0, gmm1, Tmap * 10, 'Tmap')
#     plt.show()
#
#     M = ot.dist(x, x)
#     M /= M.max()
...
#     plot1D_mat(gmm0, gmm1, G0, 'OT matrix G0')
#     plt.show()
#
#     print('left hier')
